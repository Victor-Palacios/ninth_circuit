...
Output (long format, 30 PDFs * 2 models * 11 features = 660 rows):
  ...
  Columns: case_id, pdf_url, model, feature, predicted, evidence, latency_ms, error, raw_output
...
OUT_COLUMNS = [
    ...
    "predicted", "evidence", "latency_ms", "error", "raw_output",
]
...
def request_raw(client: OpenAI, model: str, pdf_text: str) -> str:
    """Raw LLM message content for one (case, model) instance, BEFORE any
    fence/<think> stripping. This exact string is what gets recorded in the
    raw_output column, so it is captured separately from parsing (below) — that
    way it survives even when the JSON fails to parse or validate."""
...
def parse_raw(raw: str) -> BaseModel:
    ...
    if result is None:
        for name in FEATURE_NAMES:
            rows.append({
                ...
                # Same raw output on every feature row for this instance.
                "raw_output": raw,
            })
        return rows
...
def load_checkpoint() -> tuple[list[dict], set[tuple[str, str]]]:
    ...
    # Tolerate checkpoints written before the raw_output column existed.
    if "raw_output" not in df.columns:
        df["raw_output"] = ""
    df["raw_output"] = df["raw_output"].fillna("")
...
def main() -> None:
    ...
    for i, row in enumerate(cases, 1):
        ...
        for model in pending_models:
            ...
            raw = ""
            try:
                raw = request_raw(client, model, text)
            ...
            rows.extend(explode_to_rows(cid, url, model, result, lat, err, raw))
            flush(rows)
