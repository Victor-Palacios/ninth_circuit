...
export default function TableFilters({ columns, filters, onFilterChange }) {
  const renderFilter = (col) => {
    ...
    const baseClass = 'w-full min-w-[70px] px-1.5 py-1 bg-surface border border-border text-text font-mono text-[11px] outline-none transition-colors focus:border-accent'
  ...
  return (
    <tr>
      {columns.map(col => (
        <th key={col} className="bg-filter-bg px-1.5 py-1 border-b-2 border-border border-r border-r-border">
          {renderFilter(col)}
        </th>
      ))}
    </tr>
